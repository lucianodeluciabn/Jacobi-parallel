/*****************************************************************************/
/* jacobi.1.5b.c parallelo: MPI + OMP (ALLGATHER  - CON GESTIONE HOTSPOT)    */
/*  					     			             */
/*****************************************************************************/
...
int main(int argc, char *argv[])
{
	...
	if(rank!=(size-1)){
		...
		int* threads_job;
		int count_i;
		int omp_rank;//rank del thread omp;
		e_num=0; e_den=0;

		#pragma omp parallel shared(A, b, x_k, x_k_1, x_tmp, job_worker, threads_job, rank, e_num, e_den, iter) private(omp_rank, k, i, aii, x_k_1_quad, x_k_1_private, count_i)
		{

			//definizione numeratore e denominatore per il
			//calcolo della condizione d'uscita				
			x_k_1_private=(double*)malloc(A.n_row*sizeof(double));//vettore della soluzione al passo k+1, utilizzato privatamente dai thread omp
			threads_job=(int*)malloc(omp_get_max_threads()*sizeof(int));//vettore condiviso dai thread omp per conoscere il work assegnato agli altri dallo scheduler
	
			//ogni thread omp ottiene il suo id
			...
			{	
				...
				//i thread omp aggiornano x_k_1 dai loro spazi privati una sola volta
				//all fine (evitando l'hotspot)
