/*********************************************************************************/
/*  jacobi.1.6b.c parallelo + OMP - (GATHER + BROADCAST - CON GESTIONE HOTSPOT)  */
/*  					     			                 */
/*********************************************************************************/
...
int main(int argc, char *argv[])
{
	...
	{	
		...
		/*****************gestione hotspot*****************/
								  //
		double* x_k_1_private;				  //
		int* threads_job=(int*)malloc(omp_get_max_threads()*sizeof(int));//
		int count_i;					  //
		int omp_rank;					  //
								  //
		/**************************************************/
		 ...
		 #pragma omp parallel shared(A_gen, b_gen, x_k, x_k_1, x_tmp, job_worker, threads_job, rank, e_num, e_den) private(omp_rank, k, i, aii, x_k_1_quad, x_k_1_private, count_i) firstprivate(iter)
		{

			/*****************gestione hotspot**********************************/
											   //
			x_k_1_private=(double*)malloc(A_gen.n_col*sizeof(double));         //
			...
			{	
				...
				{
					x_k_1_private[i] = b_gen[i];
		
					for(k=A_gen.lr[i]; A_gen.lc[k]<i; k++)
					{
						x_k_1_private[i] = x_k_1_private[i] - ( A_gen.le[k]*x_k[A_gen.lc[k]] );
					}
					...
					x_k_1_private[i] = x_k_1_private[i]/aii;
				...
				threads_job[omp_rank]=count_i+1;
				...
				#pragma omp barrier						          //
													  // 
				if(omp_rank!=0)								  //
					for(i=threads_job[omp_rank-1]; i<threads_job[omp_rank]; i++)	  //
						x_k_1[i]=x_k_1_private[i];				  //
				else									  //
					for(i=rank*job_worker; i<threads_job[omp_rank]; i++)		  //
						x_k_1[i]=x_k_1_private[i];				  //
				#pragma omp barrier							  //
													  //
				/**************************************************************************/
	...
	//==================================================workers===================================================//
		...
		#pragma omp parallel shared(A, b, x_k, x_k_1, x_tmp, job_worker, threads_job, rank, e_num, e_den) private(omp_rank, k, i, aii, x_k_1_quad, x_k_1_private, count_i) firstprivate(iter)
		{

			/*****************gestione hotspot**********************************/
											   //
			x_k_1_private=(double*)malloc(A.n_row*sizeof(double));		   //
			omp_rank=omp_get_thread_num();					   //
										           //
			/*******************************************************************/
			...
			{	
				...
				{										  //
					...
					x_k_1_private[i] = b[i];						  //
														  //
					for(k=A.lr[i]; A.lc[k]<i+(job_worker*rank); k++)			  //
					{									  //
						x_k_1_private[i] = x_k_1_private[i] - ( A.le[k]*x_k[A.lc[k]] );	  //
					}									  //
														  //
				...
				if(omp_rank!=0)								  //
					...
					for(i=0; i<threads_job[omp_rank]; i++)				  //
						x_k_1[i]=x_k_1_private[i];				  //
				#pragma omp barrier							  //
													  //
				/**************************************************************************/
